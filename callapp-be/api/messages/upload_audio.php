<?php
// Audio upload endpoint for messages
...
try {
    error_log("=== Starting audio upload process ===");
    
    // Check if file was uploaded
    if (!isset($_FILES['audio']) || $_FILES['audio']['error'] !== UPLOAD_ERR_OK) {
        $errorCode = $_FILES['audio']['error'] ?? 'unknown';
        error_log("File upload error. Error code: " . $errorCode);
        sendResponse(false, "Audio upload failed. Please try again. Error code: " . $errorCode);
    }
    ...
    error_log("Upload audio request - Chat ID: " . $chatId . ", Sender ID: " . $senderId);
    error_log("Audio file info: " . print_r($audio, true));
    ...
    // Validate file type
    $allowedTypes = ['audio/wav', 'audio/mpeg', 'audio/mp4', 'audio/aac', 'audio/ogg'];
    if (!in_array($audio['type'], $allowedTypes)) {
        sendResponse(false, "Invalid file type. Only WAV, MP3, MP4, AAC, and OGG audio files are allowed. Received: " . $audio['type']);
    }
    
    // Validate file size (max 10MB for audio)
    if ($audio['size'] > 10 * 1024 * 1024) {
        sendResponse(false, "File size too large. Maximum allowed size is 10MB. Received: " . ($audio['size'] / 1024 / 1024) . " MB");
    }
    ...
    $filename = uniqid('aud_', true) . '.' . $extension;
    ...
    if (!file_exists($audio['tmp_name'])) {
        error_log("Source file does not exist: " . $audio['tmp_name']);
        sendResponse(false, "Source file does not exist");
    }
    
    error_log("Source file size: " . filesize($audio['tmp_name']));
    error_log("Source file permissions: " . substr(sprintf('%o', fileperms($audio['tmp_name'])), -4));
    
    // Move uploaded file
    error_log("Attempting to move file from " . $audio['tmp_name'] . " to " . $filePath);
    $moveResult = move_uploaded_file($audio['tmp_name'], $filePath);
    
    if (!$moveResult) {
        $error = error_get_last();
        error_log("Failed to move uploaded file. Error: " . print_r($error, true));
        error_log("Source: " . $audio['tmp_name'] . ", Destination: " . $filePath);
        error_log("Source exists: " . (file_exists($audio['tmp_name']) ? "Yes" : "No"));
        ...
        sendResponse(false, "Failed to save audio file. Check server permissions.");
    }
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, ?, 'audio', ?, ?, ?, NOW())
    ");
    $result = $stmt->execute([$chatId, $senderId, '', $fileUrl, $audio['name'], $audio['size']]);
    ...
    error_log("Audio uploaded successfully. Message ID: " . $messageId);
    sendResponse(true, "Audio uploaded successfully", $message);
} catch (Exception $e) {
    error_log("Error uploading audio: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    sendResponse(false, "Error uploading audio: " . $e->getMessage());
}
?>